...
TEST_CASE("MTRSim::WriteMTRSimODFFilter: Round-trip through /ODF_best (back-compat)", "[MTRSim][WriteMTRSimODFFilter]")
{
  ...
  const fs::path outputFile = fs::temp_directory_path() / fmt::format("mtrsim_export_roundtrip_{}.h5", MTRSIM_GET_PID());
  ...
  // Import the reference file.
  {
    ...
    Arguments readArgs;
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_InputFile_Key, std::make_any<FileSystemPathParameter::ValueType>(inputFile));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(std::string{"/ODF_best"}));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_OutputImageGeometry_Key, std::make_any<DataPath>(imageGeomPath));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_CellAttrMatName_Key, std::make_any<DataObjectNameParameter::ValueType>(cellAttrMatName));
  ...
  {
    componentPaths.push_back(cellAttrMatPath.createChildPath(fmt::format("component_{}", c)));
  }
  ...
  // Export to the temp file.
  {
    ...
    Arguments writeArgs;
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_OutputFile_Key, std::make_any<FileSystemPathParameter::ValueType>(outputFile));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(std::string{"/ODF_best"}));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_InputImageGeometry_Key, std::make_any<DataPath>(imageGeomPath));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_ODFComponents_Key, std::make_any<MultiArraySelectionParameter::ValueType>(componentPaths));
...
TEST_CASE("MTRSim::WriteMTRSimODFFilter: axis mapping preserves phi1-PHI-phi2 layout", "[MTRSim][WriteMTRSimODFFilter][AxisMapping]")
{
  UnitTest::LoadPlugins();
  ...
  {
    CreateImageGeometryAction::DimensionType dims = {k_NumPhi2, k_NumPHI, k_NumPhi1};
    CreateImageGeometryAction::OriginType origin = {0.0f, 0.0f, 0.0f};
    CreateImageGeometryAction::SpacingType spacing = {k_SpacingPhi2Deg, k_SpacingPHIDeg, k_SpacingPhi1Deg};
    CreateImageGeometryAction geomAction(imageGeomPath, dims, origin, spacing, cellAttrMatName, IGeometry::LengthUnit::Unknown);
    auto geomResult = geomAction.apply(dataStructure, IDataAction::Mode::Execute);
  ...
  // Create the single Float64 component array with ZYX tuple shape {nPhi1, nPHI, nPhi2}.
  {
    ...
    auto arrayResult = arrayAction.apply(dataStructure, IDataAction::Mode::Execute);
  ...
  {
    auto& componentArray = dataStructure.getDataRefAs<Float64Array>(componentPath);
  ...
  const fs::path outputFile = fs::temp_directory_path() / fmt::format("mtrsim_axis_mapping_{}.h5", MTRSIM_GET_PID());
  std::error_code removeErr;
  if(fs::exists(outputFile))
  {
    fs::remove(outputFile, removeErr);
...

  {
    ...
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(std::string{"/ODF_best"}));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_InputImageGeometry_Key, std::make_any<DataPath>(imageGeomPath));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_ODFComponents_Key, std::make_any<MultiArraySelectionParameter::ValueType>(std::vector<DataPath>{componentPath}));
  ...
  REQUIRE(meta.dimsPhi1PHIPhi2 == std::array<int64_t, 3>{static_cast<int64_t>(k_NumPhi1), static_cast<int64_t>(k_NumPHI), static_cast<int64_t>(k_NumPhi2)});
  REQUIRE(std::abs(meta.spacingDegPhi1PHIPhi2[0] - static_cast<double>(k_SpacingPhi1Deg)) < 1e-9);
  REQUIRE(std::abs(meta.spacingDegPhi1PHIPhi2[1] - static_cast<double>(k_SpacingPHIDeg)) < 1e-9);
  REQUIRE(std::abs(meta.spacingDegPhi1PHIPhi2[2] - static_cast<double>(k_SpacingPhi2Deg)) < 1e-9);
...
TEST_CASE("MTRSim::WriteMTRSimODFFilter: zero components rejects at preflight", "[MTRSim][WriteMTRSimODFFilter][ErrorPath]")
{
  UnitTest::LoadPlugins();
  ...
  Arguments args;
  args.insertOrAssign(WriteMTRSimODFFilter::k_OutputFile_Key, std::make_any<FileSystemPathParameter::ValueType>(fs::temp_directory_path() / "mtrsim_should_not_be_written.h5"));
  args.insertOrAssign(WriteMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(std::string{"/ODF_best"}));
  args.insertOrAssign(WriteMTRSimODFFilter::k_InputImageGeometry_Key, std::make_any<DataPath>(DataPath({"ODF"})));
  args.insertOrAssign(WriteMTRSimODFFilter::k_ODFComponents_Key, std::make_any<MultiArraySelectionParameter::ValueType>(std::vector<DataPath>{}));
...
void runDistinctPrefixRoundTrip(const std::string& fixtureName, const std::string& readPrefix, const std::string& writePrefix, const std::string& tempStem)
{
  ...
  const fs::path inputFile = fs::path(fmt::format("{}/{}", unit_test::k_DataDir.view(), fixtureName));
  REQUIRE(fs::exists(inputFile));
...

  {
    ...
    Arguments readArgs;
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_InputFile_Key, std::make_any<FileSystemPathParameter::ValueType>(inputFile));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(readPrefix));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_OutputImageGeometry_Key, std::make_any<DataPath>(imageGeomPath));
    readArgs.insertOrAssign(ReadMTRSimODFFilter::k_CellAttrMatName_Key, std::make_any<DataObjectNameParameter::ValueType>(cellAttrMatName));
...

  {
    ...
    Arguments writeArgs;
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_OutputFile_Key, std::make_any<FileSystemPathParameter::ValueType>(outputFile));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_Hdf5PathPrefix_Key, std::make_any<StringParameter::ValueType>(writePrefix));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_InputImageGeometry_Key, std::make_any<DataPath>(imageGeomPath));
    writeArgs.insertOrAssign(WriteMTRSimODFFilter::k_ODFComponents_Key, std::make_any<MultiArraySelectionParameter::ValueType>(componentPaths));
...
TEST_CASE("MTRSim::WriteMTRSimODFFilter: Round-trip blank_ODF with distinct prefix", "[MTRSim][WriteMTRSimODFFilter]")
{
  runDistinctPrefixRoundTrip("blank_ODF.h5", "/blank_ODF", "/blank_ODF_exported", "mtrsim_blank_rt");
}

TEST_CASE("MTRSim::WriteMTRSimODFFilter: Round-trip uniform_ODF with distinct prefix", "[MTRSim][WriteMTRSimODFFilter]")
{
  runDistinctPrefixRoundTrip("uniform_ODF.h5", "/uniform_ODF", "/uniform_ODF_exported", "mtrsim_uniform_rt");
}
