...
TEST_CASE("ODFFileIO::readODFComponents against exemplar", "[odffileio]") {
  ...
  const std::size_t expectedSize =
      static_cast<std::size_t>(72 * 36 * 72); // 186624
  for (std::size_t ci = 0; ci < comps.size(); ++ci) {
    ...
    const double sum =
        std::accumulate(comps[ci].values.begin(), comps[ci].values.end(), 0.0);
...
TEST_CASE("ODFFileIO round-trip read/write/read", "[odffileio]") {
  ...
  const std::vector<ODFFileComponent> srcComps =
      readODFComponents(k_ExemplarFile);
  ...
  writeODFFile(outPath, srcMd.dimsPhi1PHIPhi2, srcMd.spacingDegPhi1PHIPhi2,
               srcComps);
  ...
  CHECK(rtMd.spacingDegPhi1PHIPhi2[0] ==
        Approx(srcMd.spacingDegPhi1PHIPhi2[0]).margin(1e-9));
  CHECK(rtMd.spacingDegPhi1PHIPhi2[1] ==
        Approx(srcMd.spacingDegPhi1PHIPhi2[1]).margin(1e-9));
  CHECK(rtMd.spacingDegPhi1PHIPhi2[2] ==
        Approx(srcMd.spacingDegPhi1PHIPhi2[2]).margin(1e-9));
