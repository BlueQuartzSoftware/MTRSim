...
TEST_CASE("ODFBuilder::accumulate throws on values-size mismatch", "[ODFBuilder]")
{
  const mtrsim::ODFBuildParams p{72, 36, 72, 5.0, false};
  std::vector<double> values(100, 0.0); // wrong size
  REQUIRE_THROWS_AS(mtrsim::accumulate({{0.1, 0.2, 0.3}}, p, values), std::invalid_argument);
}
...
TEST_CASE("ODFBuilder::normalize is a no-op when normalizer is zero", "[ODFBuilder]")
{
