...
// -----------------------------------------------------------------------------
WriteMTRSimODF::WriteMTRSimODF(DataStructure& dataStructure, const IFilter::MessageHandler& mesgHandler, const std::atomic_bool& shouldCancel, WriteMTRSimODFInputValues* inputValues)
...
{
  m_MessageHandler(IFilter::Message::Type::Info, fmt::format("Writing MTRSim ODF file: {}", m_InputValues->outputFile.string()));
  ...
  const std::array<double, 3> spacingDegPhi1PHIPhi2 = {static_cast<double>(spacingXYZ[2]), static_cast<double>(spacingXYZ[1]), static_cast<double>(spacingXYZ[0])};
  ...
  for(std::size_t c = 0; c < m_InputValues->odfComponents.size(); ++c)
  {
    ...
    const auto& componentArray = m_DataStructure.getDataRefAs<Float64Array>(m_InputValues->odfComponents[c]);
    const auto& store = componentArray.getDataStoreRef();

    m_MessageHandler(IFilter::Message::Type::Info, fmt::format("Packing component_{} ({} values) from '{}'", c, store.getSize(), m_InputValues->odfComponents[c].toString()));
  ...
  try
  {
    mtrsim::writeODFFile(m_InputValues->outputFile, dimsPhi1PHIPhi2, spacingDegPhi1PHIPhi2, components, m_InputValues->hdf5PathPrefix);
  } catch(const std::exception& e)
  {
    return MakeErrorResult(-12110, fmt::format("ODF write failed: {}", e.what()));
  }
