...
// -----------------------------------------------------------------------------
MTRSim::MTRSim(DataStructure &dataStructure,
...
Result<> MTRSim::operator()() {
  ...
  const auto &odfGeom = m_DataStructure.getDataRefAs<ImageGeom>(
      m_InputValues->inputOdfGeometryPath);
  ...
  for (const auto &path : m_InputValues->odfComponentPaths) {
    ...
    components.push_back(mtrsim::gridToODFComponent(
        values, n1, nPHI, n2, static_cast<double>(odfSpacing[2]),
        static_cast<double>(odfSpacing[1]),
        static_cast<double>(odfSpacing[0])));
  }
  ...
  m_MessageHandler(
      ...
      "Running MTR simulation (this may take a while for large volumes)...");
  ...
  const DataPath cellAm = m_InputValues->outputGeometryPath.createChildPath(
      m_InputValues->cellAttrMatName);
  auto &mtrIds = m_DataStructure.getDataRefAs<Int32Array>(
      cellAm.createChildPath(m_InputValues->mtrIdsArrayName));
  auto &eulers = m_DataStructure.getDataRefAs<Float32Array>(
      cellAm.createChildPath(m_InputValues->eulersArrayName));
...
// -----------------------------------------------------------------------------
Result<> MTRSim::applyPolarColoring(const mtrsim::MTRSimResult &sim,
                                    const DataPath &cellAttrMatPath) {
  const std::size_t N = sim.phi1.size();
  Eigen::VectorXd phi1 = Eigen::Map<const Eigen::VectorXd>(
      sim.phi1.data(), static_cast<Eigen::Index>(N));
  Eigen::VectorXd phi = Eigen::Map<const Eigen::VectorXd>(
      sim.phi.data(), static_cast<Eigen::Index>(N));
  Eigen::VectorXd phi2 = Eigen::Map<const Eigen::VectorXd>(
      sim.phi2.data(), static_cast<Eigen::Index>(N));

  mtrsim::IPFMapper mapper{mtrsim::CrystalSystem::HCP};
  const std::vector<mtrsim::RGBColor> colors = mapper.eulerToColors(
      phi1, phi, phi2, {0.0, 0.0, 1.0}, mtrsim::IPFColorScheme::MatLab);

  auto &rgb = m_DataStructure.getDataRefAs<UInt8Array>(
      cellAttrMatPath.createChildPath(m_InputValues->polarColorsArrayName));
