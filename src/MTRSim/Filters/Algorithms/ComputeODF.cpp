...
{
public:
  AccumulateWorker(const Float32Array& eulers, const Int32Array& phases, const UInt32Array& crystalStructures, const MaskCompareUtilities::MaskCompare* mask, const mtrsim::ODFBuildParams& params,
                   std::vector<double>& masterValues, std::size_t& masterContributingCount, std::size_t& masterTotalDeposits, std::size_t& masterFailureCount, std::mutex& mergeMutex,
                   const std::atomic_bool& shouldCancel, ProgressMessageHelper& progressHelper)
  : m_Eulers(eulers)
  , m_Phases(phases)
  ...
  , m_MasterFailureCount(masterFailureCount)
  , m_MergeMutex(mergeMutex)
  ...
  void operator()(const Range& range) const
  {
    ...
    auto progressMessenger = m_ProgressHelper.createProgressMessenger(std::chrono::milliseconds(500));
    const std::size_t crystalStructTuples = m_CrystalStructures.getNumberOfTuples();
    ...
    for(std::size_t i = range.min(); i < range.max(); ++i)
    {
      ...
      if(phase <= 0 || static_cast<std::size_t>(phase) >= crystalStructTuples)
      {
        progressMessenger.sendProgressMessage(1);
        continue;
      }
      ...
      const ebsdlib::EulerDType inputEu(static_cast<double>(m_Eulers[3 * i + 0]), static_cast<double>(m_Eulers[3 * i + 1]), static_cast<double>(m_Eulers[3 * i + 2]));
      const ebsdlib::Matrix3X3D Gpassive = inputEu.toOrientationMatrix().toGMatrix();
...
// -----------------------------------------------------------------------------
ComputeODF::ComputeODF(DataStructure& dataStructure, const IFilter::MessageHandler& mesgHandler, const std::atomic_bool& shouldCancel, ComputeODFInputValues* inputValues)
...
Result<> ComputeODF::operator()()
{
  ...
  const DataPath outArrayPath = m_InputValues->outputImageGeometry.createChildPath(m_InputValues->cellAttrMatName).createChildPath(m_InputValues->componentName);
  ...
  const mtrsim::ODFBuildParams params{m_InputValues->nphi1, m_InputValues->nPHI, m_InputValues->nphi2, m_InputValues->binSizeDeg, m_InputValues->applySmoothing};

  if(numVoxels == 0)
  {
    m_MessageHandler(IFilter::Message::Type::Info, "No input voxels — output ODF array left at all zeros.");
  ...
  if(m_InputValues->outputUnits == 1ULL && masterContributingCount != 0)
  {
    ...
    m_MessageHandler(IFilter::Message::Type::Info, "Output units: MUD (per-PHI-row sin(PHI) Jacobian applied).");
  }
  else
  {
