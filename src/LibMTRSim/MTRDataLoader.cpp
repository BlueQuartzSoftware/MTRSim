...
std::vector<double> readMultiColumn(const std::string &path, int &outNRows,
                                    int &outNCols) {
  std::ifstream f(path);
  if (!f.is_open()) {
    throw std::runtime_error("MTRDataLoader: cannot open file: " + path);
...
EBSDData MTRDataLoader::load(const std::string &directoryPath) {
  ...
  if (static_cast<int>(yVals.size()) != N) {
    throw std::runtime_error(
        "MTRDataLoader: X_Position and Y_Position row counts differ");
  }

  // ── Euler angles (N × 3)
  // ─────────────────────────────────────────────────────
  int nRowsEuler = 0, nColsEuler = 0;
  const std::vector<double> eulerFlat =
      readMultiColumn(sep + "EulerAngles.csv", nRowsEuler, nColsEuler);
  if (nRowsEuler != N || nColsEuler < 3) {
    throw std::runtime_error(
        "MTRDataLoader: EulerAngles.csv must have N rows × 3 columns");
  }
  ...
  if (static_cast<int>(parentVals.size()) != N) {
    throw std::runtime_error(
        "MTRDataLoader: ParentIds.csv row count differs from N");
  }
  ...
  if (static_cast<int>(boolVals.size()) != N) {
    throw std::runtime_error(
        "MTRDataLoader: BoolMTR.csv row count differs from N");
  }
