...
//
// The MATLAB switch selects a specific topology based on AR_index =
// num_components. Cases 1, 2, 5 handle exotic 2D topologies.  For the standard
// 3-component simulation (and any N != 1,2,5) the "otherwise" branch runs,
// implementing the general N-component all-touching topology with N-1
...
AssignmentRuleThresholds AssignmentRule::selectThresholds(const std::vector<double>& volumeFractions)
{
  ...
  // ──────────────────────────── No bisection: this component covers the
  // remainder of the sample space.
  {
...
//
// For each voxel (row of z), a component_list is initialised to
// {1..numComponents}. A component is eliminated (zeroed) if any of its Gaussian
