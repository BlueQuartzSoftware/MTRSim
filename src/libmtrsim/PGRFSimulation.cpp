// Port of PGRF_simulation.m — plurigaussian random field simulation orchestrator.
// Daniel M. Sparkman, 08/28/2013.
...
PGRFResult PGRFSimulation::run(const SimulationParams& params)
{
  ...
  spdlog::info("PGRFSimulation: grid {}x{}x{} = {} voxels, {} components, {} latent fields", nx, ny, nz, N, numComponents, numGaussians);
  ...
  for(int h = 0; h < numGaussians; ++h)
  {
    ...
    if(thetaRow.size() < 3)
    {
      throw std::invalid_argument("PGRFSimulation: each thetaList row must have 3 elements [theta_x, theta_y, theta_z]");
    }
