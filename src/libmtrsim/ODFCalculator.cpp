...
ODFComponent ODFCalculator::compute(const Eigen::VectorXd& phi1in, const Eigen::VectorXd& phiIn, const Eigen::VectorXd& phi2in, double degSpacing)
{
  ...
  // ── Obtain all symmetric Euler angles (HCP, matching MATLAB default) ──────
  CrystalSymmetry cs{CrystalSystem::HCP};
  const Eigen::MatrixXd symAngles = cs.expand(phi1in, phiIn, phi2in);
  const int N = static_cast<int>(symAngles.rows()); // N_input × 12
