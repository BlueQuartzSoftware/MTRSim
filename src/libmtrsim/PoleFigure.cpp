...
// ─────────────────────────────────────────────────────────────────────────────
// fromODF — port of convert_ODF_to_PF.m (direct computation, no prebuilt tables).
//
// For each non-zero ODF bin (phi1, PHI, phi2) with value v:
//   1. Compute the [0001] c-axis direction in sample frame:
//        h_sample = G^T * [0,0,1] = last row of G = [sin(phi1)*sin(PHI), −cos(phi1)*sin(PHI), cos(PHI)]
//   2. Stereographic projection from south pole (centre = [0001] up, PHI = 0):
//        X = h_x / (1 + h_z),  Y = h_y / (1 + h_z)
//   3. Intensity weight (Euler-space → pole-figure Jacobian, matching MATLAB formula):
//        weight = v * 4π² / (dphi1 * dphi2 * |cos(PHI − dPHI/2) − cos(PHI + dPHI/2)|)
...
PoleFigureData PoleFigure::fromODF(const ODFComponent& component, double degSpacing)
{
  ...
  {
    throw std::invalid_argument("PoleFigure::fromODF — ODFComponent bin vectors have inconsistent sizes");
  }

  // ── Euler-space bin spacings ─────────────────────────────────────────────────
  // dphi1 = dphi2 = dPHI = degSpacing * π / 180 for the standard layout.
  ...
  for(int m = 0; m < nTotal; ++m)
  {
    ...
    // G[2,0] = sin(phi1)*sin(PHI), G[2,1] = −cos(phi1)*sin(PHI), G[2,2] = cos(PHI)
    ...
    // Singularity when h_z → −1 (PHI → π): skip those bins.
