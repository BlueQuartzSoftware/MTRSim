...
Eigen::VectorXd primeBasesForDim(int n) {
  ...
  const int limit = std::max(
      static_cast<int>(5.0 * n * std::log(static_cast<double>(n) + 1.0) / 4.0) +
          20,
      20);
...
QSimVN::QSimVN(std::mt19937_64 &rng) : m_Rng(rng) {}
...
QSimVN::ChlrdrResult QSimVN::chlrdr(const Eigen::MatrixXd &r,
  ...
  for (int k = 0; k < n; ++k) {
    ...
    if (pivotIdx > k) {
      ...
      if (p - 1 >= k + 1) {
        ...
        c.col(k).segment(k + 1, segLen) =
            c.row(p).segment(k + 1, segLen).transpose();
    ...
    if (ckk > ep * static_cast<double>(k + 1)) {
      ...
      for (int i = k + 1; i < n; ++i) {
        ...
        const int len = i - k;
        c.row(i).segment(k + 1, len) -=
            c(i, k) * c.col(k).segment(k + 1, len).transpose();
      }

      // Conditional mean for use in future pivot search
      if (std::abs(dem) > ep) {
        y(k) = (std::exp(-am * am / 2.0) - std::exp(-bm * bm / 2.0)) /
               (sqtp * dem);
      } else {
        if (am < -10.0) {
          y(k) = bm;
...
double QSimVN::mvndns(int n, const Eigen::MatrixXd &ch, double ci, double dci,
...
std::pair<double, double> QSimVN::compute(int m, const Eigen::MatrixXd &r,
                                          const Eigen::VectorXd &a,
  ...
  const int ns = 12;
  const int nv =
      std::max(m / ns, 1); // integer division matches MATLAB floor(m/ns)
  ...
  for (int i = 1; i <= ns; ++i) {
    ...
    if (std::abs(d) > 0.0) {
      e = std::abs(d) *
          std::sqrt(1.0 + (e / d) * (e / d) * static_cast<double>(i - 2) /
