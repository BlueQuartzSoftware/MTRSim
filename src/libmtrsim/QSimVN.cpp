...
// Original copyright: Alan Genz, All rights reserved (BSD-style, see matlab/qsimvn.m).
// Adaptation: Daniel M. Sparkman 10/01/2013.
...
// Inverse normal CDF (probit) — Peter Acklam's rational polynomial approximation.
// Replaces boost::math::erfc_inv with no external dependency.
// Max absolute error < 1.15e-9 over (0, 1).
// Reference: P. J. Acklam, "An algorithm for computing the inverse normal cumulative
...
double QSimVN::mvndns(int n, const Eigen::MatrixXd& ch, double ci, double dci, const Eigen::VectorXd& x, const Eigen::VectorXd& a, const Eigen::VectorXd& b) const
{
  ...
  for(int k = 1; k < n; ++k)
  {
    ...
    // MATLAB: s = ch(i, 1:i-1) * y(1:i-1)  →  C++: ch.row(k).head(k) . y.head(k)
