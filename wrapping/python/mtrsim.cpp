...
#include "LibMTRSim/AssignmentRule.hpp"
#include "LibMTRSim/GPGenerator.hpp"
#include "LibMTRSim/IPFMapper.hpp"
#include "LibMTRSim/MTRDataLoader.hpp"
#include "LibMTRSim/ODFCalculator.hpp"
#include "LibMTRSim/ODFSampler.hpp"
#include "LibMTRSim/PGRFSimulation.hpp"
#include "LibMTRSim/PoleFigure.hpp"
#include "LibMTRSim/QSimVN.hpp"
#include "LibMTRSim/SimulationParams.hpp"
...
PYBIND11_MODULE(mtrsim, m)
{
  ...
  // discriminate between the two MATLAB-port colour pipelines)
  // -------------------------------------------------------------------------
  py::enum_<CrystalSystem>(m, "CrystalSystem")
      .value("HCP", CrystalSystem::HCP,
             "Hexagonal close-packed (maps to "
             "ebsdlib::CrystalStructure::Hexagonal_High)")
      .value("FCC", CrystalSystem::FCC,
             "Face-centred cubic (maps to "
  ...
  // -------------------------------------------------------------------------
  // CrystalSymmetry binding intentionally removed — symmetry expansion now
  // routes through EbsdLib's `LaueOps` directly inside C++ consumers
  // (ODFCalculator, ComputeODF). Python users that need symmetry expansion
  // should use EbsdLib's Python bindings (or the IPFMapper output, which
  // already applies the appropriate Laue group).
  // -------------------------------------------------------------------------
